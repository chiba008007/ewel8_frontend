...
class ExamEAIAApiService {
  async getEAIA(data: object) {
    try {
      return await httpAuth.post("/api/exam/getEAIA", data);
    } catch (e) {
      return false;
  ...
  setEAIA(data: object) {
    return httpAuth.post("/api/exam/setEAIA", data);
  }
  editEAIA(data: object) {
    return httpAuth.post("/api/exam/editEAIA", data);
  }
...
export default new ExamEAIAApiService();
